...
def display_images_with_coco_annotations(image_paths, annotations, display_type='both', colors=None):
  ...
  for ax, img_path in zip(axs.ravel(), image_paths):
    ...
    for ann in img_annotations:
      ...
      if display_type in ['seg_rle', 'both']:
        seg_rle = ann['segmentation']
        binary_mask = maskUtils.decode(seg_rle)
...
# Get all image files
image_dir = 'v1coco-segmentation/test'
all_img_files = [os.path.join(image_dir, img['file_name']) for img in annotations['images']]
random_images_files = random.sample(all_img_files, 4)

display_type = 'seg_rle'
display_images_with_coco_annotations(random_images_files, annotations, display_type)